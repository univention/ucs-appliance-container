...
MAJOR=5; MINOR=2; PATCH=4; IMAGE="univention/univention-corporate-server"; TAG="latest"; \
...
```bash
declare -A roles[master]="primary directory node" roles[slave]="replica directory node" roles[backup]="backup directory node" roles[member]="managed node"; \
MAJOR=5; MINOR=2; PATCH=4; IMAGE="univention/univention-corporate-server"; TAG="latest"; APPS="samba4"; \
for role in ${!roles[*]}; do \
